...
export default function ForgotPassword({ status, urls = {} }) {
    ...
    const submit = (event) => {
        ...
        post(urls.send || '/password/email');
    };

    return (
        <AuthFrame title="Recuperar acceso" copy="Te enviaremos un enlace para crear una nueva contrasena." home={urls.home}>
            <Head title="Recuperar contrasena" />
            {status && <div className="mb-5 rounded-2xl bg-green-50 p-4 text-sm font-bold text-green-700">{status}</div>}
...
function AuthFrame({ title, copy, home, children }) {
    return (
        <main className="flex min-h-screen items-center justify-center bg-[linear-gradient(135deg,#f8fafc_0%,#eef4ff_50%,#f8fafc_100%)] px-4 py-10 text-slate-950">
            <section className="w-full max-w-md rounded-[2rem] border border-white/70 bg-white/90 p-6 shadow-[0_24px_70px_rgba(15,23,42,0.14)] backdrop-blur md:p-8">
                ...
                <p className="mb-8 text-sm leading-relaxed text-slate-600">{copy}</p>
                {children}
            </section>
        </main>
    );
