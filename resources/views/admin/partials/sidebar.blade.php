@php
    ...
    $isAdmin = $sidebarUser?->role_id == config('constantes.idAdmin');
    $isAffiliate = $sidebarUser?->role_id == config('constantes.idAffiliate');
@endphp
        ...
        <div class="hidden md:block p-4 border-t border-[#262626] bg-[#050505]">
            <div class="flex items-center mb-4 px-2">
                <div class="w-8 h-8 rounded-full bg-[#171717] flex items-center justify-center text-white font-bold border border-[#262626] text-xs">
                    {{ substr($sidebarUser->name, 0, 1) }}
                </div>
                <div class="ml-3 overflow-hidden">
                    <p class="text-[11px] font-bold text-white truncate">{{ $sidebarUser->name }}</p>
                    <p class="text-[9px] text-[#737373] font-medium uppercase tracking-tighter">
                    {{ $isAdmin ? 'Administrator' : 'Socio Afiliado' }}
                    </p>
                </div>
            </div>

            <a href="{{ $isAdmin ? route('admin.profile.edit') : route('socios.profile.edit') }}" class="mb-3 flex items-center justify-center rounded-lg border border-white/10 py-2 text-[10px] font-bold uppercase tracking-widest text-[#a1a1a1] transition hover:border-[#fcca00]/40 hover:text-[#fcca00]">
